...
- **`GitHubApiOptions`** — inner `OptionsFactory` that configures the `APIRequestContext` with GitHub's base URL and
  auth headers.
- **`SecretsProvider`** — static utility that resolves test secrets such as `GITHUB_USER` and `GITHUB_API_TOKEN` via an
...
Test secrets such as `GITHUB_USER` and `GITHUB_API_TOKEN` are resolved through `SecretsProvider`
(`oscarvarto.mx.secrets`), which uses an env-var-first strategy:
